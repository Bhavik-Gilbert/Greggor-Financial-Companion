...
def edit_monetary_account_view(request: HttpRequest, pk: int) -> HttpResponse:
    ...
    if request.method == "POST":
        form: forms.ModelForm = MonetaryAccountForm(request.POST, form_type=account_type, user=user, instance=this_monetary_account)
        if form.is_valid():
                form.save(instance=this_monetary_account)
                return redirect("dashboard")
    else:
        form: forms.ModelForm = MonetaryAccountForm(form_type=account_type, user=user, instance=this_monetary_account)
