...
class ManagementSeedTestCase(ManagementTestCase):
    ...
    def _assert_successfully_unseed(self, seeded_users :QuerySet[User]):
        ...
        seeded_user_accounts: QuerySet[Account] = Account.objects.filter(user__in=seeded_users)
        seeded_user_categories: QuerySet[Category] = Category.objects.filter(user__in=seeded_users)
        seeded_user_emails: str = []
        self.assertEqual(User.objects.filter(email__endswith='@gfc.org').count(), 0)
        ...
        self.assertEqual(QuizScore.objects.filter(user__in=seeded_users).count(), 0)
        self.assertEqual(BankAccount.objects.filter(user__in=seeded_users).count(), 0)
        self.assertEqual(PotAccount.objects.filter(user__in=seeded_users).count(), 0)
        ...
        self.assertEqual(UserTarget.objects.filter(user__in=seeded_users).count(), 0)
        self.assertEqual(AccountTarget.objects.filter(account__in = seeded_user_accounts).count(), 0)
        self.assertEqual(CategoryTarget.objects.filter(category__in=seeded_user_categories).count(), 0)
        self.assertEqual(UserGroup.objects.filter(owner_email__in=seeded_user_emails).count(), 0)
    ...
    def test_successfully_seed_and_unseed(self) -> None:
        ...
        seeded_users: QuerySet[User] = User.objects.filter(email__endswith='@gfc.org')
