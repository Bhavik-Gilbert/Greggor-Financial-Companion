...
class QuizQuestionModelTestCase(ModelTestCase):
    ...
    def test_valid_potential_answer_can_be_520_characters_or_shorter(self):
        for potential_answer_index in range(1, len(
            ...
            setattr(self.test_model, f"potential_answer_{potential_answer_index}", "c" * 520)
            self._assert_model_is_valid()
            setattr(self.test_model, f"potential_answer_{potential_answer_index}", f"potential_answer_{potential_answer_index}")
    ...
    def test_invalid_cannot_have_same_response_in_potential_questions(self):
        for potential_answer_index in range(1, len(
            ...
            for different_potential_answer_index in range(1, len(
                ...
                with self.assertRaises(Exception) as raised:
                    setattr(self.test_model, f"potential_answer_{potential_answer_index}", "c")
                    setattr(self.test_model, f"potential_answer_{different_potential_answer_index}", "c")
                    self._assert_model_is_invalid()
                self.assertEqual(IntegrityError, type(raised.exception))
                
                setattr(self.test_model, f"potential_answer_{potential_answer_index}", f"potential_answer_{potential_answer_index}")
                setattr(self.test_model, f"potential_answer_{different_potential_answer_index}", f"potential_answer_{different_potential_answer_index}")

    def test_invalid_potential_answer_cannot_be_greater_than_520_characters(
            self):
        for potential_answer_index in range(1, len(
                self.test_model.get_potential_answers()) + 1):
            setattr(self.test_model, f"potential_answer_{potential_answer_index}", "c" * 521)
            self._assert_model_is_invalid()
            setattr(self.test_model, f"potential_answer_{potential_answer_index}", f"potential_answer_{potential_answer_index}")
    ...
    def test_invalid_potential_answer_cannot_be_empty(self):
        for potential_answer_index in range(1, len(
                self.test_model.get_potential_answers()) + 1):
            setattr(self.test_model, f"potential_answer_{potential_answer_index}", "")
            self._assert_model_is_invalid()
            setattr(self.test_model, f"potential_answer_{potential_answer_index}", f"potential_answer_{potential_answer_index}")
