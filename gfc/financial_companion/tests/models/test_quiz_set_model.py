...
class QuizSetModelTestCase(ModelTestCase):
    ...
    def test_valid_set_exists_returns_true_when_question_set_already_exists(self):
    ...
    def test_valid_set_exists_returns_false_when_question_set_already_exists(self):
    ...
    def test_valid_get_set_from_questions_returns_set_when_exists(self):
        question_list: list[QuizQuestion] = self.test_model.questions.all()
        get_set_from_questions: QuizSet = QuizSet.get_set_from_questions(question_list)
        self.assertFalse(get_set_from_questions is None)
    
    def test_valid_get_set_from_questions_returns_none_when_does_not_exist(self):
        question_list: list[QuizQuestion] = QuizQuestion.objects.all()
        get_set_from_questions: QuizSet = QuizSet.get_set_from_questions(question_list)
