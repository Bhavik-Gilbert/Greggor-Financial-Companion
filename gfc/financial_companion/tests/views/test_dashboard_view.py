...
class DashboardViewTestCase(ViewTestCase):
    ...
    def test_dashboard_displays_desired_info(self):
        ...
        self.assertContains(response, self.recent[0].category.name.capitalize())
        self.assertContains(response, self.recent[0].amount)
        self.assertContains(response, self.recent[0].sender_account.name.capitalize())
        self.assertContains(response, self.recent[0].receiver_account.name.capitalize())
