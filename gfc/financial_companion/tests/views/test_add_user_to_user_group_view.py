...
class AddUserToUserGroupViewTestCase(ViewTestCase):
    ...
    def setUp(self) -> None:
        self.url: str = reverse('add_user_to_user_group', kwargs={"group_pk": 1})
    ...
    def test_get_add_user_to_user_group_redirects_when_not_logged_in(self) -> None:
    ...
    def test_unsuccessful_add_user_to_user_group_due_to_invalid_group_pk(self) -> None:
        self._login(self.user)
        url: str = reverse('add_user_to_user_group', kwargs={"group_pk": 1000})
        self.assertFalse(self.user_group.members.contains(self.user_two))
        before_count: int = self.user_group.members_count()
        response: HttpResponse = self.client.post(url, self.form_input, follow=True)
        after_count: int = self.user_group.members_count()
    ...
    def test_unsuccessful_add_user_to_user_group_due_to_invalid_user(self) -> None:
        self._login(self.user)
        self.assertFalse(self.user_group.members.contains(self.user_two))
    ...
    def test_unsuccessful_add_user_to_user_group_due_to_incorrect_email(self) -> None:
        self._login(self.user)
        self.assertFalse(self.user_group.members.contains(self.user_two))
