...
class ParseStatementPDFClassTestCase(HelperTestCase):
    ...
    def setUp(self):
        ...
        self.uploaded_bank_statement: TemporaryUploadedFile = self._get_upload_file(
            bank_statement_path)
    ...
    def _assert_filled_object_data(self):
        ...
        self.assertTrue(
            self.bank_statement_parser.transaction_type is not None)
    ...
    def test_invalid_empty_get_transactions_from_pdf_statement(self):
        empty_uploaded_bank_statement: TemporaryUploadedFile = TemporaryUploadedFile(
            "empty.pdf", "application/binary", 0, 'utf-8')
        with self.assertRaises(Exception) as raised:
            self.bank_statement_parser.get_transactions_from_pdf_statement(
                empty_uploaded_bank_statement.temporary_file_path())
        self.assertEqual(ValueError, type(raised.exception))

    def test_invalid_unclean_get_transactions_from_pdf_statement(self):
        unclean_bank_statement_path: str = os.path.join(
            "financial_companion", "tests", "data", "invalid_bank_statement.pdf")
        unclean_uploaded_bank_statement: TemporaryUploadedFile = self._get_upload_file(
            unclean_bank_statement_path)
        with self.assertRaises(Exception) as raised:
            self.bank_statement_parser.get_transactions_from_pdf_statement(
                unclean_uploaded_bank_statement.temporary_file_path())
    ...
    def test_valid_get_dataframe_list_statement_column_expense_indexes_separate_income_expense_columns(
            self):
        income, expense, fail = self.bank_statement_parser.get_dataframe_list_statement_column_expense_indexes(
            self.dataframe_list)
        self.assertEqual(income, -3)
        self.assertEqual(expense, -2)
        self.assertFalse(fail)

    def test_valid_get_dataframe_list_statement_column_expense_indexes_same_income_expense_columns(
            self):
        columns: list[str] = ["Date", "Description", "Move", "Balance"]
        move_money: list[str] = ["-1.00", "2.00", "-3.00"]
        dataframe: pd.DataFrame = pd.DataFrame(list(
            zip(self.dates, self.description, move_money, self.balance)), columns=columns)
    ...
    def test_invalid_get_dataframe_list_statement_column_invalid_index_expense_column_data(
            self):
    ...
    def test_valid_get_transaction_from_dataframe_list(self):
        transactions: list[dict[str, Any]] = self.bank_statement_parser.get_transactions_from_dataframe_list(
            self.dataframe_list, self.indexes)
        self.assertEqual(len(transactions), 9)

    def test_valid_get_transactions_from_dataframe(self):
        transactions: list[dict[str, Any]] = self.bank_statement_parser.get_transactions_from_dataframe(
            self.dataframe, self.indexes)
        self.assertEqual(len(transactions), 3)

    def test_valid_get_transactions_from_dataframe_row(self):
        transaction: list[dict[str, Any]] = self.bank_statement_parser.get_transaction_from_dataframe_row(
            self.dataframe.iloc()[0], self.indexes)
    ...
    def test_valid_add_new_transaction_filled_new_transaction(self):
        ...
        transactions: dict[str, Any] = self.bank_statement_parser.add_new_transaction(
            [], new_transaction)
    ...
    def test_valid_set_expense_and_income_columns_correct_way_around_columns_are_correct_way_around(
            self):
        ...
        transactions_before: list[dict[str, Any]] = [
            transaction_1, transaction_2]
        self.assertEqual(
            transactions_before[1]["transaction_type"],
            TransactionType.EXPENSE)
        transactions_after: list[dict[str, Any]] = self.bank_statement_parser.set_expense_and_income_columns_correct_way_around(
            transactions_before)
        self.assertEqual(
            transactions_after[1]["transaction_type"],
            TransactionType.EXPENSE)

    def test_valid_set_expense_and_income_columns_correct_way_around_columns_are_wrong_way_around(
            self):
        transaction_1: dict[str, Any] = self._get_transaction()
        transaction_2: dict[str, Any] = self._get_transaction()
        ...
        dataframe: pd.DataFrame = pd.DataFrame(
            list(
                zip(
                    ...
                    self.balance)),
            columns=columns)
        self.bank_statement_parser.set_initial_balance_from_dataframe(
            dataframe, self.indexes)
        self.assertTrue(self.bank_statement_parser.balance is not None)
    ...
    def test_valid_set_balance_from_datataframe_row_change_balance(self):
        ...
        self.bank_statement_parser.set_balance_from_datataframe_row(
            self.dataframe.iloc()[0], self.indexes)
    ...
    def test_valid_set_balance_from_datataframe_row_empty_balance_field(self):
        ...
        self.bank_statement_parser.balance: float = 100
        before_balance: int = self.bank_statement_parser.balance
        self.bank_statement_parser.set_balance_from_datataframe_row(dataframe.iloc()[
                                                                    0], self.indexes)
        after_balance: int = self.bank_statement_parser.balance
    ...
    def test_valid_set_date_from_datataframe_row_change_date(self):
        ...
        self.bank_statement_parser.set_date_from_datataframe_row(
            self.dataframe.iloc()[0], self.indexes)
    ...
    def test_valid_set_date_from_datataframe_row_empty_date_field(self):
        ...
        before_date: int = self.bank_statement_parser.date
        self.bank_statement_parser.set_date_from_datataframe_row(
            dataframe.iloc()[0], self.indexes)
        after_date: int = self.bank_statement_parser.date
        self.assertEqual(before_date, after_date)

    def test_valid_set_description_from_datataframe_row_change_description_first_value(
            self):
        before_description: list[str] = self.bank_statement_parser.description
        self.bank_statement_parser.set_description_from_datataframe_row(
            self.dataframe.iloc()[0], self.indexes)
    ...
    def test_valid_set_description_from_datataframe_row_change_description_second_value(
            self):
    ...
    def test_valid_set_description_from_datataframe_row_empty_description_field(
            self):
    ...
    def test_valid_set_all_data_from_dataframe_row(self):
        self._assert_empty_object()
        self.bank_statement_parser.set_all_data_from_dataframe_row(
            self.dataframe.iloc()[0], self.indexes)
        self._assert_filled_object()
        ...
        dataframe: pd.DataFrame = pd.DataFrame(
            list(
                zip(
                    ...
                    self.balance)),
            columns=self.columns)
        self.bank_statement_parser.set_all_data_from_dataframe_row(dataframe.iloc()[
                                                                   0], self.indexes)
        self.assertTrue(self.bank_statement_parser.balance is not None)
        self.assertTrue(self.bank_statement_parser.date is None)
        self._assert_filled_object_data()
    ...
    def test_valid_set_all_data_from_dataframe_row_empty_description_field(
            self):
        self._assert_empty_object()
        description: list[str] = [np.nan, np.nan, np.nan]
        dataframe: pd.DataFrame = pd.DataFrame(
    ...
    def test_valid_set_all_data_from_dataframe_row_empty_in_money_out_money_fields(
            self):
        self._assert_empty_object()
        in_money: list[str] = [np.nan, np.nan, np.nan]
        out_money: list[str] = [np.nan, np.nan, np.nan]
        dataframe: pd.DataFrame = pd.DataFrame(list(zip(
            self.dates, self.description, out_money, in_money, self.balance)), columns=self.columns)
        self.bank_statement_parser.set_all_data_from_dataframe_row(dataframe.iloc()[
                                                                   0], self.indexes)
        self._assert_filled_object(False)
        self.assertTrue(self.bank_statement_parser.amount is None)
    ...
    def test_valid_set_amount_and_transaction_type_from_datataframe_row_change_amount_and_transaction_type_income(
            self):
        ...
        self.bank_statement_parser.set_amount_and_transaction_type_from_datataframe_row(
            self.dataframe.iloc()[0], self.indexes)
    ...
    def test_valid_set_amount_and_transaction_type_from_datataframe_row_change_amount_and_transaction_type_expense(
            self):
        before_amount: float = self.bank_statement_parser.amount
        before_transaction_type: str = self.bank_statement_parser.transaction_type
        self.bank_statement_parser.set_amount_and_transaction_type_from_datataframe_row(
            self.dataframe.iloc()[1], self.indexes)
        after_amount: float = self.bank_statement_parser.amount
        after_transaction_type: str = self.bank_statement_parser.transaction_type
        self.assertTrue(before_amount != after_amount)
        self.assertTrue(before_transaction_type != after_transaction_type)
    ...
    def test_valid_set_amount_and_transaction_type_from_datataframe_row_empty_in_money_out_money_fields(
            self):
        before_amount: float = self.bank_statement_parser.amount
        before_transaction_type: str = self.bank_statement_parser.transaction_type
        in_money: list[str] = [np.nan, np.nan, np.nan]
