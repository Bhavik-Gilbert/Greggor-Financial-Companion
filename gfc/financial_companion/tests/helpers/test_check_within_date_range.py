...
class CheckWithinDateRangeHelperFunctionTestCase(HelperTestCase):
    ...
    def _assert_range_is_invalid(self, start_date: date, end_date: date, current_date: date):
        check_range: bool = check_within_date_range(start_date, end_date, current_date)
        self.assertFalse(check_range)
    
    def _assert_range_is_valid(self, start_date: date, end_date: date, current_date: date):
        check_range: bool = check_within_date_range(start_date, end_date, current_date)
    ...
    @freeze_time("2023-01-09 00:00:00")
    def test_valid_start_date_swaps_with_end_date_when_greater_than_start_date(self):
        current_date: date = date.today()
        start_date: date = datetime(2023, 1, 10).date()
        end_date: date = datetime(2023, 1, 8).date()
        self._assert_range_is_valid(start_date, end_date, current_date)
