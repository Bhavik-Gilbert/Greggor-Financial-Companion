...
class CurrencyConversionHelperClassTestCase(HelperTestCase):
    ...
    def setUp(self) -> None:
        super().setUp()
        self.conversion: CurrencyConversion = CurrencyConversion(
            CurrencyType.USD, CurrencyType.GBP, 0.1, datetime.now())

    def test_valid_can_convert(self) -> None:
        self.assertTrue(
            self.conversion.can_convert(
                CurrencyType.USD,
                CurrencyType.GBP))
    ...
    def test_original_direction_convert(self) -> None:
        self.assertEqual(
            0.1, self.conversion.convert(
                1, CurrencyType.USD, CurrencyType.GBP))

    def test_original_direction_convert(self) -> None:
        self.assertEqual(
            10, self.conversion.convert(
                1, CurrencyType.GBP, CurrencyType.USD))
...
class StoredCurrencyConverterHelperClassTestCase(HelperTestCase):
    ...
    def test_valid_has_valid_conversion(self) -> None:
        self.assertTrue(
            self.converter.has_valid_conversion(
                CurrencyType.USD,
                CurrencyType.GBP))
        self.assertTrue(
            self.converter.has_valid_conversion(
    ...
    def test_invalid_get_conversion(self) -> None:
        self.assertTrue(
            self.converter.get_conversion(
                CurrencyType.USD,
                CurrencyType.CAD) is None)
        self.assertTrue(
            self.converter.get_conversion(
                CurrencyType.CAD,
                CurrencyType.GBP) is None)
