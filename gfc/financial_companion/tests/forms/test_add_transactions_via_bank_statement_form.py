...
class AddTransactionsViaBankStatementFormTestCase(FormTestCase):
    ...
    def test_invalid_bank_statement_must_be_pdf(self):
        image_file_path: str = os.path.join(
            "financial_companion", "tests", "data", "dragon.jpeg")
        self.form_input["bank_statement"]: TemporaryUploadedFile = self._get_upload_file(
            image_file_path)
        form: AddTransactionsViaBankStatementForm = AddTransactionsViaBankStatementForm(
            self.form_input, self.form_input, user=self.user)
        self.assertFalse(form.is_valid())

    def test_valid_account_currency_accepts_all_values_in_currency_type_enum(
            self):
    ...
    def test_invalid_account_currency_does_not_accept_value_not_in_currency_type_enum(
            self):
    ...
    def test_invalid_account_does_not_belong_to_user(self):
        self.form_input["account"]: PotAccount = PotAccount.objects.exclude(
            user=self.user).first()
        self.assertTrue(hasattr(self.form_input["account"], "user"))
